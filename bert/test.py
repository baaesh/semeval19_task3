...
def test(model, dataloader, criterion, args, device):
    model.eval()
    acc, loss, size = 0, 0, 0
    ...
    acc_, prec, rec, f1 = getMetrics(preds, labels, args.class_size)
    acc /= size
    return loss, acc, f1
...
def getMetrics(predictions, ground, num_classes):
    ...
    for c in range(1, num_classes):
        ...
        print("    Class %s : Precision : %.3f, Recall : %.3f, F1 : %.3f" % (label_list[c], precision, recall, f1))
